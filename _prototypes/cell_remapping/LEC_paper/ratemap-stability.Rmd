...
# _only_keep.xlsx')
df <- readxl::read_excel('e:\\Julian\\LEC_final_figures\\duration_correct\\output\\df_full_LEC_filtered.xlsx')
...
# command ot run: & "C:\Program Files\R\R-4.5.2\bin\Rscript.exe" -e "rmarkdown::render('ratemap-stability.Rmd')"     
#JULIAN CHANGE
df <- df %>% filter(score == 'whole')
...
#   # Define the formula string
#   # julian is this the issue? comment out for now
...
#     formula_str <- paste(dependent_var, " ~ group + (1|name) + session_id + group:session_id")
...
#JULIAN CHANGE
write.csv(summary_table, "./summary_tables/whole_NONvsANT_summary_table.csv")
# end chunk 8
```
...
# Print the results
#JULIAN CHANGE
write.csv(results, "./LEC_model_report/NONvsANT_quantile_indiv_beta_whole.csv")
print(results)
# end chunk 13
```
